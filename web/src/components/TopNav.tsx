...
export function TopNav({ onBridge }: { onBridge: () => void }) {
  return (
    <header className="flex h-14 items-center gap-6 border-b border-line bg-bg px-4">
      {/* logo block */}
          ...
          {({ account, chain, openAccountModal, openChainModal, openConnectModal, mounted }) => {
            ...
            return (
              <button
                ...
                className="flex items-center gap-2 rounded-lg border border-line bg-raised px-4 py-2 text-sm font-semibold hover:border-brand"
              >
                <WalletGlyph />
                {!connected
                  ? "Connect Wallet"
...
function NavItem({
  ...
  if (soon) {
    return (
      <span
        title="Coming soon"
        className="cursor-not-allowed rounded-md px-3 py-1.5 text-muted/50 select-none"
      >
        {children}
      </span>
    );
  ...
  return (
    <div ref={ref} className="relative">
      <button
        onClick={() => setOpen((v) => !v)}
        aria-label="Settings"
        className={`flex size-9 items-center justify-center rounded-lg border border-line hover:border-brand ${
      ...
      {open && (
        <div className="absolute right-0 top-full z-30 mt-1.5 w-72 rounded-lg border border-line bg-overlay p-3 text-xs shadow-2xl">
